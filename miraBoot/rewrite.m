...
rwDynSt   == (globalEnv, [exception], num)
rwLexSt   == locInfo
...
|| also take this opportunity to inline fully-saturated calls to stdApply and stdCompose, since they are used
|| so frequently to make computation pipelines, and it is cheaper to do it here than in the general
|| inline pass
rewriteAst k (Eap e1 e2) st
    = (k (Eap e1 e2) $st_bind mkCall) st        || rewrite sub-expressions e1 & e2, then make them into an Ecall
      where
        ...
        checkInline (Ecall c [f])    e = checkInline f e,           if c == stdApply
        checkInline (Ecall c [f, g]) e = Ecall f [checkInline g e], if c == stdCompose
...
|| some inlining rewrites not being performed -- look at removing and also removing builtinSel from desugar of LHS patterns
...
rewriteAst k (Efatbar p e1 e2) st
    ...
    = (Efatbar p e1' e2', st2'), otherwise
      where
        ...
        (e1r, x3)  = astRewriteTDCont repFail e1' e2'           || replace matching Efails in e1 with e2

        || replace Efails matching fn in e1 with e2
        repFail k (Efail (Evar n)) er = (er, er), if n == fn
        repFail k e er                = k e er

        || is the expr a call to stdCaseFail or stdMatchFail?
        isSDF (Ecall f args) = f == stdCaseFail \/ f == stdMatchFail
        isSDF (Eap f arg)    = f == stdCaseFail \/ f == stdMatchFail    || rewrite from Eap -> Ecall may not have happened, yet
        isSDF e              = False
...
rewriteAst k (Elet x1 defs x2 e) st
    = (lets, st2)
      where
        ...
        sccs          = (findSCCs . makeDependencyGraph . makeDefnDependencyList) defs'
        ...
        mkLet [n] e
            = Elet isRec (m_singleton n d') [] e
              where
                d     = fromJust (m_lookup n defs')
                isRec = s_member n (view defnAnFree d)
            ...
            = Elet True (foldl addRecEntry m_empty ns) [] e
              where
                addRecEntry m n
                    = m_insert n d' m
                      where
                        d' = (set defnAnRec True . fromJust . m_lookup n) defs'
...
rewriteAst k a st
    ...
    = k a st,          otherwise
      where
        ...
        fixup = over defnAnFree (s_delete stdCaseFailName . s_delete stdMatchFailName)

|| rewrite of desugared code:
||   rewrite simple accessors in Ecase patterns
||   inline saturated compose operator
...
rewrite genv
    ...
    = ex_errs   errs,        otherwise
      where
        ...
        sccs          = (findSCCs . makeDependencyGraph . makeDefnDependencyList) defs1
        ...
        markRec defs [n]
            = m_adjust setRec n defs
              where
                setRec d = set defnAnRec (s_member n (view defnAnFree d)) d

        || mark mutally-recursive defns as recursive
        markRec defs ns
            = foldr (m_adjust (set defnAnRec True)) defs ns
