...
|| running
mst_runState :: maybeState * ** -> * -> (maybe **, *)
mst_runState m s = st_runState m s

mst_evalState :: maybeState * ** -> * -> maybe **
mst_evalState m s = st_evalState m s

mst_execState :: maybeState * ** -> * -> *
mst_execState m s = st_execState m s
    ...
    = go
      where
        go s = alt (mst_runState ma s)
               where
                 alt (Nothing, x) = mst_runState mb s
                 alt ra           = ra
