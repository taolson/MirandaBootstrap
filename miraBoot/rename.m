...
rnLexSt   == (locInfo, nameMap, s_set num)
...
addRnEnv n1 n2 = over rnEnv (m_insert n1 n2)

|| lookup a name in the rnEnv
lookupRnEnv :: name -> rnSt -> maybe name
lookupRnEnv n = m_lookup n . view rnEnv
...
renameAst k (Elet rec defs x e) st
    ...
    = k (Elet rec defs' x e) st2, otherwise
      where
        ...
        g              = makeDependencyGraph (map mkDep rdefs)
        morder         = dependencyOrder g                              || rdefs must be handled in dependency-order
...
renameDef n st
    ...
    = (n,  st),                      otherwise
      where
        ...
        s          = getName n
        s'         = s,        if null pfx \/ pfx $isPrefixOf s
                   = pfx ++ s, otherwise
...
tvarRef k (Tvar n) st
    = (Tvar n, st')
      where
        st' = addRnErr (UnboundTvar n) st, if ~s_member n (view rnTenv st)
            = st,                          otherwise
...
tvarDef (Tvar n) st
    = (Tvar n, st')
      where
        st' = addRnErr (RepeatedTvar n) st, if s_member n (view rnTenv st)
            = over rnTenv (s_insert n) st, otherwise
