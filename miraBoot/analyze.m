...
||
|| TODO: this could probably be written more simply with a bottom-up traversal
||       However, there would be issues with merging the info from two branches
||       of a tree when going up -- this is done currently by capturing the
||       incoming free/use info and clearing them before traversing down into
||       the new branch, then merging the outgoing info from that branch back
||       into the captured incoming values before returning
...
azDynSt   == ([exception], b_bag name, num)
azLexSt   == locInfo
...
checkFail n st
    = addAzWarn NonExhaustive st, if n == stdCaseFailName
    = st,                         otherwise
...
addUse n st = over azUses (b_insert n) st

|| add a use of a Tname to the use count
addTUse :: name -> azSt -> azSt
addTUse n st
    = over azUses (b_insert n) st, if ~isConstructor n
    = st,                          otherwise
...
delUses ns = over azUses (converse (foldr b_deleteKey) ns)

|| add to the complexity count
addCpx :: num -> azSt -> azSt
...
setUseCount k a st
    ...
    = k a st,    otherwise
      where
        n        = defnName a
        count    = b_findWithDefault 0 n (view azUses st)
...
analyzeAst k (Efatbar p e1 e2) st = (k (Efatbar p e1 e2) $st_left st_modify (delUses (patNames p) . incCpx)) st
analyzeAst k (Evar n)          st = (k (Evar n)          $st_left st_modify (addUse n . incCpx . checkFail n)) st
analyzeAst k (CaseAlt p e)     st = (k (CaseAlt p e)     $st_left st_modify (delUses (patNames p) . incCpx)) st
analyzeAst k (Qgen pats e)     st = (k (Qgen pats e)     $st_left st_modify (delUses (concatMap patNames pats) . incCpx)) st
analyzeAst k (Qrecur p e1 e2)  st = (k (Qrecur p e1 e2)  $st_left st_modify (delUses (patNames p) . incCpx)) st
analyzeAst k (Pctor n vars)    st = (k (Pctor n vars)    $st_left st_modify (addUse n . incCpx)) st
analyzeAst k (Tname n args)    st = (k (Tname n args)    $st_left st_modify (addTUse n)) st

|| handle extra info and updates around rewriting defns
analyzeAst k a st
    = (k a $st_bind update) st', if isDefn a
        ...
        update d st
            = (d', st2)
              where
                ...
                free  = b_deleteKey n uses, if isDefAbs d       || remove all def name from abstype free refs
                      = b_delete n uses,    if isDefType d      || remove def name (Tname) occurrence, since it is not a free ref
                      = uses,               otherwise
                d'    = (set defnAnCpx cpx . set defnAnFree (b_keysSet free)) d
                st2   = (set azCpx (inCpx + cpx + 1) . set azUses (b_union inUses free)) st1
...
analyzeExprCpx :: expr -> globalEnv -> num
analyzeExprCpx e genv
    = view azCpx st
      where
        (e', st) = astRewriteLex analyzeAst e initAzSt
