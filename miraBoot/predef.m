...
|| standard library and stream library file names
filePathSeparator, stdlibModName, strmModName :: string
filePathSeparator = "/"
stdlibModName     = "stdlib"
strmModName       = "stream"

stdlibPath :: [string]
stdlibPath =  split (hd filePathSeparator) "../lib/"

|| generate a unique internal name
|| use a prefix of "`" for any names you don't want to appear in exception reports or
|| profiling reports
genName :: string -> num -> (num, name)
genName s gen = (gen + 1, Internal s gen)

|| generate a list of unique internal names to replace an existing list
genNames :: string -> num -> [*] -> (num, [name])
genNames s = mapAccumL gn where gn g x = genName s g

|| generate a unique private name
genPrivateName :: string -> string -> num -> (num, name)
genPrivateName mn s gen = (gen + 1, Private mn ((shows s . shows  "_" . showsnum gen) ""))

|| generate a unique internal variable
genVar :: string -> num -> (num, expr)
genVar s gen = (gen', Evar n) where (gen', n) = genName s gen

|| generate a unique internal pattern variable
genPvar :: string -> num -> (num, pat)
genPvar s gen = (gen', Pvar n) where (gen', n) = genName s gen

|| generate a unique type variable
genTvar :: num -> (num, texpr)
genTvar gen = (gen + 1, Tvar gen)

genTvars :: num -> [*] -> (num, [texpr])
genTvars = mapAccumL gt where gt gen x = genTvar gen

|| generate an expression from a location
genLoc :: location -> expr
genLoc (mn, il, ic) = makeTuple [Estring mn, EprimInt il, EprimInt ic]
...
makeStdlibName s = Qualified stdlibModName s
...
makeTuple :: [expr] -> expr
makeTuple [elt] = elt                   || single element
makeTuple elts  = foldl Eap ntuple elts || tuple of elements
                  where
...
makeTupleCtorType n
    ...
    = foldr Tarr (Ttuple tvars) tvars,                                                   otherwise
      where
        msize = stripPrefix "Tuple" (getName n)
        tsize = numval (fromJust msize)
...
|| make a function name for a tuple, given the base function name and the tuple type
...
makeTupleFnName :: string -> texpr -> name
makeTupleFnName s (Ttuple ts) = Unqualified (s ++ "tuple" ++ shownum (#ts))

|| make Tuple comparison function and show function vars from their types
makeTupleCmp, makeTupleShow :: texpr -> expr
...
makeBuiltinCtorDef n
    = DefFn n [] (builtinPack n tag aty) an
      where
        ...
        aty  = 0,                   if n == builtinNilName \/ n == builtinUnitName
             = 2,                   if n == builtinConsName
             = builtinTupleArity n, if isBuiltinTuple n
             = err,                 otherwise

        tag  = 1,                   if n == builtinConsName
             = 0,                   otherwise
...
stdSysMsg   = makeStdlibType "sys_message"
...
builtinNilName     = makeBuiltinName "Nil"
builtinConsName    = makeBuiltinName ":"
builtinUnitName    = makeBuiltinName "Unit"     || use "Unit" instead of "()" to disambiguate with unit type
builtinExitName    = makeBuiltinName "exit#"

builtinTupleName :: num -> name
builtinTupleName n = makeBuiltinName ((shows "Tuple" . showsnum n) "")

builtinTupleArity :: name -> num
builtinTupleArity (Builtin n) = numval ns where Just ns = stripPrefix "Tuple" n
...
builtinEnv
    = env
      where
        env = foldr ins m_empty [ builtinTypeType, builtinTypeUnit, builtinTypeWord,
        ...
        ins (Tname n ts) env = (m_insert (unqualifyName n) n . m_insert n n) env
        ins (Evar  n)    env = (m_insert (unqualifyName n) n . m_insert n n) env
    ...
    = (foldl insertSpec m_empty . map p_spec) specs
      where
        specs = [
                ...
                , "systemCmd#        :: word# -> word#"                 || execute the shell command specified by a byteStream and return its result
                ]
        insertSpec m d = m_insert (defnName d) d m
