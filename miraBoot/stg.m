...
%export codeObj codePtr stgCode heapInfo heapObj heapPtr stgHeap stackObj stgStack continuation
        ...
        selApplyPtr blackHolePtr localEnv stgInsn showValue showOpr showStgInsn showHeapObj showStackObj
...
codePtr  == num                                 || index into the stgCode map
...
heapInfo ::=
    ...
    Hfun    codePtr! num! |       || function with arity num, list values are free variable values of the function
    Hthunk  codePtr!      |       || thunk (arity 0)
    Hfwd    heapPtr!      |       || forward to another heapPtr (used when an Hthunk is updated to its value)
    ...
    Hfbalt  codePtr! num! |       || fatbar alternate, with captured stack pointer to unwind to

    || these heapObjects are data only (no codePtr)
    Hstream [char]        |       || a byteStream holding a packed list of chars
    Harray  num!          |       || a contiguous array of size num, starting with the next word
    Hvalue                        || a value of an array (kept in the heapObj value list)

heapObj     == (heapInfo, [value])
closure     == heapObj                              || type synonym for a heapObj
closureInfo == (codePtr, num, bool, bool)           || codePtr, arity, free vars empty, isDefFun
heapPtr     == num                                  || index into the stgHeap map
...
heapObjWords :: heapObj -> num
heapObjWords (info, env)
    = hwords info + #env
      where
        hwords (Hstream s) = 1          || NOTE: should be "stringWords s + 1", but we are currently overwriting Hstreams
                                        || with new stream values (variable size), so the interp GC can get confused as to
                                        || the size when tracing, so it is fixed at 1 for interp.  The runtime.c garbage
                                        || collector has its own heapObjWords function which uses the right value for compiled
                                        || code.
        hwords (Harray n)  = n + 1      || tag word plus N contiguous words for the values
        hwords Hvalue      = 1          || an array value has no tag word
...
value ::= Word num! | Caddr codePtr! | Haddr heapPtr!
...
stgInsn ::=
    ...
    RetEnv                              |    || copy env to Reg 0,1, args[0 .. r1-1], pop a continuation closure from the stack, and enter it
    ...
    PopEnv      num                     |    || pop N values from the stack into the environment for a continuation closure

    || heap manipulation
    AllocFrame  num                     |    || allocate num contiguous heapPtrs and store the base heapPtr in the framebase register
    Fclosure    ref codePtr num [opr]   |    || ref := addr of new Hfun closure
    ...
    Cmp         ref opr opr             |    || ref := [opr1] $cmp [opr2]   -- 0 == EQ, 1 == LT, 2 == GT
...
showStgInsn :: stgInsn -> string
showStgInsn = show

showHeapObj :: heapObj -> string
showHeapObj = show

showStackObj :: stackObj -> string
showStackObj = show
...
bdLexSt  == (localEnv, localCls, locInfo, num, num, bool)
...
initBdSt genv
    = initResources ((genv, code, heap), (m_empty, m_empty, emptyLocInfo, 0, 0, False))
      where
        code     = (m_fromList special, #special)       || code has special generic routines pre-installed
        heap     = (m_empty, 0)
...
|| alloc a num resource in bdSt strictly
allocResource :: lens bdSt num -> num -> state bdSt num
allocResource lns n
    = bd_view lns $st_bind inc
      where
        inc r
            = r' $seq bd_set lns r' $st_right st_pure r
              where r' = r + n
...
updateCode cp obj
    = bd_over bdCodeMap (m_insert cp obj) $st_right st_pure cp
...
makeOpr (EprimString s)
    = allocHeap (stringWords s + 1) $st_bind mkByteStream
      where
        mkByteStream hp = updateHeap hp (Hstream s, []) $st_right (st_pure . Imm . Haddr) hp

makeOpr e        = error ("makeOpr: unhandled expr: " ++ showAstRaw e)

|| allocate a reg
allocReg :: state bdSt ref
allocReg = pair Reg $st_fmap allocResource bdRegGen 1
...
allocFrmSlots :: num -> state bdSt ref
allocFrmSlots n = pair Frm $st_fmap allocResource bdFrmGen (n + 2)
...
makeRef (Pvar n) st
    = (r, st2)
      where
        (r, st1) = allocReg st
        st2      = over bdLenv (m_insert n (Ref r)) st1
...
inLexEnv lns pvs fns ma
    = bd_view lns $st_bind newLex
      where
        ...
        newLex lex
            = bd_over bdLenv (addEntries (arefs ++ erefs)) $st_right ma $st_left bd_set lns lex
              where
...
|| saving and restoring the whole lexical state, and adding an AllocFrame if required
inDefEnv :: locInfo -> [pat] -> [name] -> state bdSt [stgInsn] -> state bdSt [stgInsn]
inDefEnv loc pvs fns ma
    = inLexEnv bdLex pvs fns mb
      where
        mb = st_modify initResources          $st_right || initialize both FrmGen and RegGen for new Defn context
             bd_over bdLoc (mergeLocInfo loc) $st_right
             ma                               $st_bind
             addFrame

        addFrame ins
            = go $st_fmap bd_view bdFrmGen
              where
                go 0 = ins
                go n = AllocFrame n : ins

|| run a build function in a new environment of Arg refs for a caseAlt,
|| saving and restoring the whole lexical state, and adding an AllocFrame if required
|| a common Env for all free vars in all CaseAlts in the Ecase has been set up at the beginning of
|| the continuation closure
inAltEnv :: [pat] -> state bdSt [stgInsn] -> state bdSt [stgInsn]
inAltEnv pvs ma
    = inLexEnv bdLex pvs [] mb
      where
        mb = st_modify initFrame $st_right      || initialize the FrmGen, but not the RegGen (maintain registers still alive)
             ma                  $st_bind
             addFrame
...
buildCodeAlt (CaseAlt (Pvar n) e)
    = st_pure [Ret r0Opr r1Opr],        if e == Evar n  || just return existing r0/r1/args
    = bd_view bdBuiltin $st_bind doAlt, otherwise
      where
        doAlt inBuiltin
            = inAltEnv [] (buildCode e),   if inBuiltin || the Pvar n has already been bound to an allocated register for builtins
            = allocReg $st_bind createAlt, otherwise
              where
                createAlt reg
                  = addClosure $st_fmap inAltEnv [] (bd_over bdLenv (m_insert n (Ref reg)) $st_right buildCode e)
                    where
                      addClosure eins = Vclosure reg unpackPtr : eins

buildCodeAlt (CaseAlt (Pctor cn vars) e)
    = inAltEnv vars (buildCode e)
...
buildCodeAltsTally alts
    = st_bind2 (bd_view bdRegGen) (bd_view bdFrmGen) doAlts
      where
        doAlts brc bfc
            = foldr go (st_pure ([], brc, bfc)) alts $st_bind setMax
              where
                go alt k
                    = bd_set bdRegGen brc $st_right bd_set bdFrmGen bfc $st_right buildCodeAlt alt $st_bind getCounts
                      where
                        getCounts ins
                            = st_bind3 (bd_view bdRegGen) (bd_view bdFrmGen) k doMax
                              where
                                doMax rc fc (bbs, mrc, mfc) = st_pure (ins : bbs, max2 mrc rc, max2 mfc fc)

                setMax (bbs, mrc, mfc)
                    = bd_set bdRegGen mrc $st_right bd_set bdFrmGen mfc $st_right st_pure bbs
...
buildCodeAlts Csingle topr [alt] = buildCodeAlt alt

buildCodeAlts Cvectored topr alts
    = buildCodeAltsTally alts $st_bind st_mapM makeCode $st_kbind mkVec
      where
              ...
              where                                                     || but it is an easy insn to have as "bottom"
                doAlt (a, bb) k
                  ...
                  = st_liftA2 (:) tst k, otherwise                      || refutable (conditional) case; add Jeq of tag
                    where
                      CaseAlt p e = a
                      tst         = Jeq topr (mkTagOpr p) $st_fmap makeCode bb

                mkTagOpr (Plit (EprimInt n))  = makeImmOpr n
                mkTagOpr (Plit (EprimChar c)) = makeImmOpr (code c)
                mkTagOpr (Pctor cn vars)      = (makeImmOpr . fromMaybe 0 . elemIndex cn . fromMaybe [] . lookupCtorNames genv) cn
                mkTagOpr p                    = topr
    ...
    = st_bind3 (bd_view bdGenv) (bd_view bdLoc) allocCode mkCont
      where
        mkCont genv loc cp
            ...
            = inDefEnv emptyLocInfo [] fns (buildCodeAlts sel r0Opr alts) $st_bind updateObj
              where
                fns           = (filter isInternal . s_toList . foldl s_union s_empty . map (converse analyzeExprFree genv)) alts
                updateObj ins = st_pure (fns, cp) $st_left updateCode cp (loc, PopEnv (#fns) : ins)

|| build the alts and the code to select from them in the env current context, returning the insns along
|| with a list of Mov insns which will move any existing Frm refs to new registers if there are any refs
|| to them in the free in the free vars, and any of the case alts allocate a frame (which would destroy
|| the current Frm references)
buildCodeAltsNoCont :: caseSel -> opr -> [caseAlt] -> state bdSt ([stgInsn], [stgInsn])
buildCodeAltsNoCont sel topr alts
    = getFrmRefs $st_bind mkAlts $st_kbind mkMovs
      where
            ...
            = st_bind3 (bd_view bdGenv) (bd_view bdLenv) (bd_view bdFrmGen) getRefs
              where
                getRefs genv lenv frc
                    = zip2 frs $st_fmap st_mapM (const allocReg) frs    || allocate a new reg for each Frm ref that needs to be moved
                      where
                        fns  = (filter isInternal . s_toList . foldl s_union s_empty . map (converse analyzeExprFree genv)) alts
                ...
                needsFrm (CaseAlt p  (Elet b defs pats e)) = True        || caseAlt has an Elet, which allocs Frm slots
                needsFrm (CaseAlt p1 (Efatbar p2 e1 e2) )  = True        || caseAlt has an Efatbar, which allocs Frm slots
                needsFrm (CaseAlt p e)                     = False

        || build the alts in the existing Env, adding bindings for any moved Frm refs
        mkAlts binfo
           = pair binfo $st_fmap inAltEnv [] (st_mapM_ addBinding binfo $st_right buildCodeAlts sel topr alts)
             where
               addBinding ((of, n), r) = bd_over bdLenv (m_insert n (Ref r))

        || add the Mov insns for any moved Frm refs
        mkMovs (binfo, ins)
            = st_pure (map mkMov binfo, ins)
              where
                mkMov ((of, n), r) = Mov r of
...
buildCodeCallBuiltin n args dst
    = go (getName n)
      where
        ...
        opDS  op  = op dst       $st_fmap opr0
        opDZS op  = op dst zero  $st_fmap opr0
...
buildCodeCall (Evar f) args
    = mkList $st_fmap buildCodeCallBuiltin f args r0Ref, if isBuiltin f & ~isConstructor f
      where
        mkList x = [x]
        ...
        mkCall fopr aoprs minfo
            = checkImm fopr
              where
                ...
                checkImm (Imm (Haddr hp)) = lookupHeap hp $st_bind checkHobj hp
                checkImm x                = checkInfo minfo
                ...
                mkDirect mhp cp arity emptyEnv isFn
                    ...
                    = st_pure [EnterInd fopr aoprs],        otherwise                   || enter closure indirectly from fopr
                      where
                        ...
                        mkOverApp         = (PushCont applyToEnvPtr aoprs2 :) $st_fmap mkCall fopr aoprs1 minfo
...
buildCode (Elet rec defs x e)
    ...
    = st_mapM allocObj defl $st_bind st_mapM createDefnCode $st_kbind createExprCode e
      where
        defl = m_toList defs
            ...
            = st_bind2 (allocFrmSlots nslots) allocCode mkObj
              where
                ...
                nslots = nfns,        if isDefFn d
                       = max2 nfns 1, otherwise         || always allocate at least 1 slot for use by Update

                mkObj rf cp
                    = st_pure (d, cp, rf, fns)             $st_left
                      bd_over bdLenv (m_insert n (Ref rf)) $st_left
                      bd_over bdLcls (m_insert n clsInfo)
                      where
            ...
            = st_bind3 (buildCode d) (st_mapM (makeOpr . Evar) fns) (bd_view bdLoc) updateObj
              where
                updateObj ins frs loc
                    = st_pure (d, cp, rf, frs) $st_left updateCode cp (loc', ins)
                      where
...
buildCode (Ecase sel (Ecall (Evar fn) args) alts)
    = mkDst $st_bind mkCode, if isBuiltin fn & ~isConstructor fn
      where
        isAlloc     n = member ["allocArray#", "allocByteStream#", "allocFileStream#"] (getName n)

        CaseAlt p e = hd alts
        singleVar   = sel == Csingle & isPvar p
        ...
        mkCode dst
            = st_liftA2 insMovs (buildCodeCallBuiltin fn args dst) buildAlts
              where
                buildAlts
                      || if the case scrutinee builtin performs an alloc, we need to build the alts in a new altEnv so that
                      || any frane slots they alloc are bound to a new AllocFrame, rather than the enclosing env's
                      || potential frame.  Otherwise, the AllocFrame and its initialization (the Fclosure/Tclosure insns)
                      || would be separated by the case scrutinee's alloc operation, leading to a potential uninitialized
                      || frame if a garbage collection was to occur
                    = pair [] $st_fmap buildCode e,                                                                      if singleVar & ~isAlloc fn
                    = inLexEnv bdBuiltin [] [] (bd_set bdBuiltin True $st_right buildCodeAltsNoCont sel (Ref dst) alts), otherwise
...
buildCode (Ecase sel e alts)
    = st_bind2 (buildCode e) buildAlts mkCase
      where
        buildAlts = inLexEnv bdBuiltin [] [] (bd_set bdBuiltin False $st_right buildCodeAltsCont sel alts)

        mkCase eis (fns, cp)
            = mkCont $st_fmap st_mapM (makeOpr . Evar) fns
              where
...
buildCode (Tname f as)
    = st_pure [Ret (makeImmOpr tag) (makeImmOpr arity)], if f == builtinPackName
      where
        EprimInt tag   = as ! 1
        EprimInt arity = as ! 2
...
buildCode (Ecall (Tname f as) args)
    = st_liftA2 mkSel (bd_view bdGenv) (buildCode e), if f == builtinSelName
      ...
      where
        ...
        mkSel genv eis
            = PushCont selPtr [makeImmOpr i] : eis
              where
                t          = (nthType i . fromJust . lookupSpec genv) cn        || get type of nth ctor arg
                selPtr     = selRetPtr,   if t == builtinTypeWord               || if we are selecting a word#, use Sel followed by a Ret
...
|| then build the primary branch with the fail var bound to a new register ref, and append it to the Sclosure insn
            ...
            = st_bind4 allocClosure
              ...
              where
                ...
                updateObj cls cp fvs ins
                    = updateCode cp (loc, ins) $st_bind mkPrim cls fvs

        mkPrim cls fvs altCp
            = mkClosure $st_fmap inLexEnv bdLenv [] [] (bd_over bdLenv (m_insert n (Ref cls)) $st_right buildCode e1)
              where
                mkClosure insns = Sclosure cls altCp fvs : insns

|| an Efail invokes the Sclosure through the Fail insn
buildCode (Efail e)
    = mkFail $st_fmap makeOpr e
      where
...
buildCode d
    ...
    = error ("unhandled expr in buildCode: " ++ showAst d), otherwise
      where
        ...
        buildCodeDef (DefFn n args e an)
            = inDefEnv loc args fns (buildCode e)
              where
        ...
        buildCodeDef (DefPat p e an)
            = mkCode $st_fmap inDefEnv loc [] fns (buildCode e)
              where
...
buildGroup genv ns
    ...
    = st_mapM allocObj ns $st_bind st_mapM createCode
      where
        ...
        allocObj n
            = st_bind2 allocCode (allocHeap nslots) (mkObj d) $st_bind insObj
              where
                ...
                mkObj d cp hp
                    = st_pure (n, cp, hp, d) $st_left updateHeap hp (mkInfo d, [])
                      where
                ...
                insObj (n, cp, hp, d)
                    = st_pure (cp, v, d) $st_left bd_over bdLenv (m_insert n (Imm v))
                      where
                        v = Haddr hp
        ...
        createCode (cp, v, d)
            = st_bind2 (buildCode d) (bd_view bdLoc) updateObj
              where
                updateObj ins loc
                    = st_pure v $st_left updateCode cp (loc', ins)
                      where
                        loc' = mergeLocInfo (defnLocInfo d) loc
...
reachableFrom ns genv
    = walk m_empty (stdBlackHoleName : ns)
      where
        walk seen [] = seen
        walk seen (n : ns)
            = walk seen ns,             if m_member n seen
...
makeStart n genv
    = error ("cannot find spec for " ++ showName n),  if isNothing mspec
    = error ("entry function must be of type IO ()"), if spec ~= ioUnitSpec
    = (start, genv'), otherwise
      where
        ioUnitSpec = Tarr builtinTypeUnit (Ttuple [builtinTypeUnit, builtinTypeUnit])   || io () is a type synonym for () -> ((), ())
        mspec      = lookupSpec genv n
        spec       = (view defnType . fromJust) mspec
        start      = Qualified (getModName n) "!start"
        fvs        = s_fromList [n, builtinUnitName, builtinExitName]
...
|| build the STG code & heap reachable from the start name, and return the heapPtr of the
|| start function, along with the stg code and heap
...
buildStg n genv
    = (view bdLenv st', view bdCode st', view bdHeap st')
      where
        ...
        sccs            = (findSCCs . makeDependencyGraph . m_toList) topLevel
        (x, st)         = st_mapM_ (buildGroup genv') sccs (initBdSt genv')
        Imm (Haddr bhp) = (fromJust . m_lookup stdBlackHoleName . view bdLenv) st
        bhTramp         = (fromJust . m_lookup blackHolePtr . view bdCodeMap) st        || get existing blackHole trampoline code obj
        bhTramp'        = setSnd bhTramp [Enter bhp []]                                 || update trampoline to enter actual blackHole closure
        st'             = over bdCodeMap (m_insert blackHolePtr bhTramp') st            || replace with new trampoline code
