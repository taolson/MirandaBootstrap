|| -*- mode: indented-text -*-
||
|| grammar.m -- Miranda expression, type, and definition grammars
...
ast ::=
        ...
        Echar       char                       |
        Estring     string                     |
        EprimInt    num                        |       || represents an unboxed Int#
        ...
        Ecase       caseSel expr [caseAlt]     |       || caseSel is case selection type for selecting a caseAlt
                                                       || of the DefData are present in definition order, allowing a table-jump to the correct CaseAlt
        ...
        || texpr
        Tname       name    [texpr]            |       || named constant type with optional list of Tvars
                                                       || (for DefSyn, DefData) or application of higher-kinded types

        Tvar        num                        |       || type variable
        Ttuple      [texpr]                    |       || n-tuple type
        Tlist       texpr                      |       || list type
        Tarr        texpr   texpr              |       || arrow (function type) e.g. num -> char
        Tstrict     texpr                      |       || strict form, used in constructor arguments

        || defn                           
        DefFn       name    [pat]  expr  anno  |       || name, vars, rhs
        DefMult     multSel [defn]             |       || container for multiple definitions of the same name
...
anno == (location, bool, num, num, s_set name, s_set num)         || loc, recursive, complexity, use count, free vars, strictness info
...
showschar c
    = (("\'" ++ showEscChar c ++ "\'") ++)
      where
        ...
        showEscChar '\v' = "\\v"
        showEscChar '\\' = "\\"
        showEscChar c    = "\\" ++ showsnum n "" , if n < 32
                         = [c],                    otherwise
                           where n = code c

showsstring :: string -> showS
showsstring s = shows (show s)
...
showsAnno (loc, rec, cpx, uses, free, strict)
    = ([] ++)
||    = shows "{" . showsRec . showsnum cpx . shows " " . showsnum uses .
||      shows " [" . interShows ", " (map showsName (s_toList free)) . shows "]" .
||      shows " [" . interShows ", " (map showsnum (s_toList strict)) . shows "]}"
||      where
...
showsLocation (mn, el, ec)
    = shows "in module " . shows mn . showsLineCol el ec
      where
        ...
        showsLineCol el ec = shows " near line " . showsnum el . shows " col " . showsnum ec
...
showsAstPrec :: num -> ast -> showS

|| atomic asts can ignore precedence
showsAstPrec x (Evar n)        = shows (showName n)
showsAstPrec x (Eint n)        = showsnum n
showsAstPrec x (Echar c)       = showschar c
showsAstPrec x (Estring s)     = showsstring s
showsAstPrec x (EprimInt n)    = showsnum n . shows "#"
...
showsAstPrec d (Ecall (Evar f) args)
    ...
    = showsInParensIf (d > apPrec) s1, otherwise
      where
        s0 = shows (showName f)
        s1 = shows (showName f ++ " ") . showsAstList args
...
showsAstPrec d (Pctor n vars)
    ...
    = showsInParensIf (d > apPrec) s,        otherwise
      where
        s = shows (showName n ++ " ") . showsAstList vars
...
showsAstPrec d a
    = showsInParensIf (d > apPrec) (go a)
      where
        ...
        go a = error ("unhandled ast in showsprec: " ++ show a)

showsOpPrec :: num -> name -> expr -> expr -> showS
showsOpPrec d op e1 e2 
    = showsInParensIf (d > prec) s
      where
        ...
        (precL, precR) = (prec, prec + 1), if assoc == AssocLeft
...
reservedNames = s_fromList ["if", "otherwise", "type", "where", "abstype", "with", "case", "of", "=", "::", "::=", "..", "|", "<-", "->"]
...
isPrefix n      = ((== AssocPrefix) . snd . opPrecAssoc) n
isVar n         = ~(isConstructor n \/ s $s_member reservedNames) where s = getName n
...
samePatType (Plit l1)     (Plit l2)     = l1 == l2
...
typeName (Ttuple tes)   = Builtin ("tuple" ++ shownum (#tes))
...
baseType (Tarr t1 t2)    = baseType t2
baseType t               = t

|| return the nth type in a Tarr texpr
nthType :: num -> texpr -> texpr
nthType n (Tarr t1 t2) = t1,                 if n <= 0
                       = nthType (n - 1) t2, otherwise
nthType n t            = t
...
defnType
    = Lens getf overf
      where
        ...
        overf fn d                    = error ("defnType: bad defn: " ++ showAst d)
...
precAssocTable :: m_map string (num, assoc)
precAssocTable
    = m_fromList
      ...
      , ("*>",  (4,  AssocRight))       || generic applicative right
...
opPrecAssoc :: name -> (num, assoc)
opPrecAssoc op
    = m_findWithDefault (apPrec, AssocRight) (getName op) precAssocTable
