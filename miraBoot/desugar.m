...
|| maximum number of constructors to perform an expansion of an Ecase to Cvectored
|| (matches enum arity of name, which is compared extensively in map lookups)
maxCaseAll = 6
...
matchVar var vars peqns
    = st_mapM (addRename var) peqns $st_bind match vars
      where
        addRename n' (Pvar n, (p, e, m))
            = st_pure (p, e, m_insert n n' m),                                if isNothing mn
...
matchCtor var vars peqns st
    ...
    = (Ecase sel (Evar var) alts', st2),           otherwise
      where
        ...
        DefSpec tf te an  = fromJust mspec
        dname             = typeName (baseType te)
        mcns              = lookupCtorNames genv ctor
        cns               = fromJust mcns
        sel               = Csingle, if #cns == 1
                          = Cvectored, if #cns <= maxCaseAll
                          = Cpriority, otherwise
        ...
        partitionByCtorName sel (cn : cns) elts st = ((cn, p1) : rest, st'), if ~null p1 \/ sel == Cvectored
                                                   = (rest, st'),            otherwise
                                                     where
                                                       cs          = getName cn
...
patCaseTree [] mr st = (((makeTuple . map Evar . m_elems) mr, m_keys mr), st)
        ...
        renameVar p rvs
            = go p
              where
                ...
                go (Pvar n)
                    = (vs, cs, mr, addDsErr (NameClash n) st),   if m_member n mr
                    = (Pvar n' : vs, cs, m_insert n n' mr, st'), otherwise
                      where
...
desugarAst k (Elet rec defs pats e) st
    = (Elet rec defs2 [] e', st2)
      where
        ...
        (Elet x2 defs1 pats' e', st1) = k (Elet rec defs pats e) st     || desugar the sub-asts of the Elet
...
desugarAst k (Econd conds) st
    = k caseTree st
      where
        ...
        mkCase (Cdefault e) rest = e    || early-out on default so that the base case Efail is not included
        mkCase (Cif tst e)  rest = Ecase Cvectored tst [CaseAlt (Pctor stdFalseName []) rest, CaseAlt (Pctor stdTrueName []) e]

|| desugar explicit case exprs (set the caseAll flag correctly)
desugarAst k (Ecase Cnone e alts) st
    = k (builtinFatbar (Ecase sel e alts') (Eap stdCaseFail (genLoc loc))) st
      where
        ...
        sortedLitAlts         = sortOn getLit alts
        sortedCtorAlts        = sortOn getCtorTag alts
        failAlt               = CaseAlt Pwildcard builtinFail
        ...
        getCtorTag  (CaseAlt (Pctor n vars) e)      = fromMaybe 0 (elemIndex (getName n) allCtors)
...
desugarDef k (DefMult Mfun defs) topLevel st
    = k (DefFn fname (map Pvar varNames) caseExpr an) st2
      where
        ...
        arity            = max (map length pats)        || use max pat as arity for all pats
        gen              = view dsGen st
        fname            = defnName d
        an               = defnAnno d
        (gen', varNames) = genNames "var" gen [1 .. arity]
        st1              = set dsGen gen' st
        (caseTree, st2)  = match varNames (zip3 pats rhss (replicate (#rhss) m_empty)) st1
...
desugarLit k e st
    ...
    = (Evar v,  st),  otherwise
      where
        mn         = view dsModName st
        gen        = view dsGen st
        mv         = m_lookup e (view dsLits st)
        v          = fromJust mv
        (gen', v') = genPrivateName mn "lit" gen
        st'        = over dsLits (m_insert e v') (set dsGen gen' st)
...
desugar genv
    ...
    = ex_errs   errs,        otherwise
      where
        ...
        pats            = view modPats m
        (defs1, x, st') = astRewriteInLet (astRewriteLex desugarAst) defs pats (initDsSt genv)
