...
getArity (Evar n) st
    = foldr lookFrame lookGenv (view nmLenv st)
      where
        ...
        lookFrame f k = (fnArity $mb_fmap m_lookup n f) $mb_alt k

|| handle polymorphic pseudo-functions pack and sel
getArity (Tname n as) st
    = Just 1,     if n == builtinSelName
    = Just nargs, if n == builtinPackName
    = Nothing,    otherwise
      where
        EprimInt nargs = as ! 2
...
normalizeAst k a st
    ...
    = (k a $st_bind update) st1, if isDefn a \/ isCaseAlt a
    = (k a $st_bind update) st2, if isLet a                             || add new lexical frame for let defns
    = k a                   st,  otherwise
      where
            ...
            = (foldl wrapLet a (view nmThunks st), set nmThunks inThunks st)
              where
                ...
                wrapLet a (DefPat p e an)
                    = Ecase Csingle e [CaseAlt p a],                if eagerCtors & isEager e
