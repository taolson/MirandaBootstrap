...
%export deriveShow deriveInstances
...
deriveShow d
    = (defSpec, defFn)
      where
        ...
        showForType (Ttuple ts)    = foldl Eap (makeTupleShow (Ttuple ts)) (map showForType ts)
        ...
        findInstName te            = (fromMaybe (Unqualified "undef") . mb_fmap snd . find ((== te) . fst)) instNames

        genDef (Tname n tas)
            = DefFn go [Pctor n (map Pvar vars)] showExpr an
              where
                ns               = Estring (getName n)
                (x, vars)        = genNames "a" 0 tas
...
deriveOrd d
    = (defSpec, defFn)
      where
        ...
        cmpForType (Ttuple ts)    = foldl Eap (makeTupleCmp (Ttuple ts)) (map cmpForType ts)
...
deriveInstances genv
    = converse (set genvMod) genv $ex_fmap ex_foldM insInstance m (shows ++ ords)
      where
        ...
        missing s d
            = isNothing (m_lookup n' defs) & ~any (match n') pats
              where
                n  = defnName d
                n' = Qualified (getModName n) (s ++ getName n)

                match n (DefPat (Pvar n') e an) = n == n'
                match n p                       = False
