...
||             and rewriting thunks which are demanded to strict case evaluations
||
...
|| 2) is wrong anyway, because a single-use DefPat could escape, anyway, allowing them to be evaluated multipled times

|| configuration
verbose        = True  || add extra info in the output
...
addDmName n = dm_over dmNames (s_insert n)
...
delDmNames ns
    = dm_over dmNames (converse (foldr s_delete) ns) $st_right
      dm_over dmMap   (converse (foldr m_delete) ns)

|| add a note to the demand state, limiting the length of the note string
addNote :: string -> state dmSt ()
addNote s
    = dm_over dmNotes ((Note, Info s', emptyLocInfo) :), if verbose
    = st_pure (),                                        otherwise
      where
        s' = limitInfo 100 s
    ...
    = st_liftA2 go (dm_view dmGenv) (dm_view dmMap)
      where
        go genv dm
            ...
            = s_fromList [0 .. arity - 1], if isBuiltin n       || builtins demand all of their arguments
            = fromMaybe s_empty mds,       otherwise
              where
                arity = fromJust (lookupArity genv n)
                mds   = m_lookup n dm $mb_alt view defnAnnoStrict $mb_fmap lookupDefn genv n
...
demandAst k (Elet rec defs pats e)
    = (dm_view dmNames $st_bind
      ...
      where
        ...
        doDefs dnsIn = astRewriteMap (astRewriteTDCont demandAst) defs $st_left dm_set dmNames dnsIn

        || find the demand for the expr (on the strict evaluation path)
        doExpr defs' = pair defs' $st_fmap astRewriteTDCont demandAst e
        ...
        mkStrictPat (defs', e')
            = dm_view dmNames $st_bind checkPat
              where
                ...
                checkPat dns
                    = st_pure noUpdLet $st_left addNote note, if noUpd
                    = st_pure updLet,                         otherwise
                      where
                        noUpd    = ~rec & isDefPat d & (isCtorCall e' \/ uses < 2 & s_member dn dns)
...
demandAst k (Ecase sel e alts)
    = st_liftA2 (Ecase sel) (astRewriteTDCont demandAst e) (dm_view dmNames $st_bind doAlts)
      where
        || compute each alt separately, then combine the separately-discovered dmNames
        doAlts dns = st_mapM (doAlt dns) alts $st_bind (combineAlts . unzip2)

        || peform demand analysis on the alt, then return it, along with its dmNames as a pair
        doAlt dns alt
            = st_liftA2 pair (dm_set dmNames dns $st_right astRewriteTDCont demandAst alt) (dm_view dmNames)

        || combine all the alt's dmNames with a set intersection
        combineAlts (alts', dnss) = st_pure alts $st_left dm_set dmNames (foldl1 s_intersect dnss )
...
demandAst k (Ecall e args)
    = doFn   $st_bind
      ...
      where
        ...
        doArgs (e, dns) = pair e $st_fmap astRewriteList (astRewriteTDCont demandAst) args $st_left dm_set dmNames dns

        checkFn (Evar f, args)
            = st_pure (Ecall (Evar f) args) $st_left
              dm_over dmNames (s_insert f)  $st_left
              lookupDemand f $st_bind checkArgs

        checkFn (e, args)       = st_pure (Ecall e args)
        checkArgs ds            = st_mapM_ (checkArg ds) (enumerate args)

        checkArg ds (i, Evar n) = dm_over dmNames (s_insert n), if s_member i ds
        checkArg ds x           = st_pure ()

|| Efatbar needs to go down the left path only for demand
demandAst k (Efatbar p e1 e2)
    = st_liftA2 (Efatbar p) (astRewriteTDCont demandAst e1) (dm_view dmNames $st_bind doAlt) $st_left delDmNames (patNames p)
      where
        doAlt dns = astRewriteTDCont demandAst e2 $st_left dm_set dmNames dns

|| delete CaseAlt patNames after discovering the demand of its expr
demandAst k (CaseAlt p e) = k (CaseAlt p e) $st_left delDmNames (patNames p)

|| add an Evar reference (thunk evaluation) to the dmNames
demandAst k (Evar n) = st_pure (Evar n) $st_left addDmName n
    ...
    = k a,                                    otherwise
      where
        doDef (DefFn n vars e an) dns
            = dm_over dmMap (m_insert n strict) $st_right     || add strictness info for n into dmMap
              delDmNames vns                    $st_right     || remove now out-of-scope var names from dmNames
              ...
              where
               ...
               findVarIdx s (i, n)
                   = s_insert i s, if s_member n dns 
                   = s,            otherwise


demandGroup :: defnMap -> [name] -> state dmSt defnMap
demandGroup defs ns
    = dm_view dmGenv $st_bind doGroup
      where
        doGroup genv
            = addDefs $st_fmap astRewriteList (astRewriteTDCont demandAst) defs'
              where
                defs'        = catMaybes (map (lookupDefn genv) ns)
                addDefs      = foldl ins defs . zip2 ns
                ins m (k, v) = m_insert k v m

demand :: globalEnv -> excpt globalEnv
demand genv
    = ex_return genv  [],    if True            || demand disabled until it is debugged
