...
%export token tokloc tokenize showToken isTerror
...
showToken (TlitInt n)    = show n
showToken (TlitChar c)   = show c
showToken (TlitString s) = show s
showToken (TprimInt n)   = show n ++ "#"
showToken (TprimChar c)  = show c ++ "#"
showToken (Tchar c)      = show c
showToken Toffside       = "OFFSIDE"
showToken Tend           = "END"
showToken (Terror s)     = "ERROR " ++ s

tokloc == (token, num, num)                   || token, line#, col#


tokenize :: string -> [tokloc]
tokenize = tokenizePos 1 1

tokError :: string -> num -> num -> [tokloc]
tokError err il ic = [(Terror err, il, ic), (Tend, il, ic)]


tokenizer == num -> num -> string -> [tokloc]  || line#, col#, input
...
tokenizeInt :: (num -> tokenizer) -> tokenizer
...
tokenizeRadix :: num -> (num -> tokenizer) -> tokenizer
tokenizeRadix radix k il ic cs
    = k n il ic' cs'
      where
