...
||
||     infix symbols can be used as identifiers in definitions, e.g. (+) :: num -> num -> num or a << b = a * 2 ^ b
...
p_identifier
    = (p_any $p_bind mkIdent) $p_alt p_unexpected
      where
...
p_symbol
    = (p_any $p_bind mkSym) $p_alt p_unexpected
      where
...
p_infix :: parser name
p_infix
    = (p_symbol $p_guard isInfix) $p_alt p_unexpected
      where
        isInfix n = ~s_member (getName n) reservedNames & ~isPrefix n

p_infix1 :: parser name
p_infix1
    = p_infix $p_guard ((~= "-") . getName)

p_prefix :: parser name
p_prefix
    = p_symbol $p_bind fpre
      where
...
p_var :: parser name
p_var
    = (p_identifier $p_alt p_inParens p_symbol) $p_guard isVar

p_constructor :: parser name
p_constructor
    = (p_identifier $p_alt p_inParens p_infix) $p_guard isConstructor

p_varOrConstructor :: parser name
p_varOrConstructor = p_var $p_alt p_constructor
...
p_literal
    = (p_any $p_bind flit) $p_alt p_unexpected
      where
...
p_parenExpr pElt
    = mkParen $p_fmap p_inParens (p_listManyOf pElt)
      where
        mkParen []  = builtinUnit  || ()
        mkParen [x] = x            || (expr)
        mkParen xs  = makeTuple xs || (a, b ..)

|| parse an explicit list of expressions
p_listExpr :: parser expr -> parser expr
p_listExpr pElt = p_inBrackets (makeList $p_fmap p_listManyOf pElt)
...
p_generator = p_liftA2 Qgen (p_listSomeOf p_pat $p_left p_lArrow) p_exp

|| parser for second form of generator, e.g. x <- 1, 2 * x ...
p_recurrence :: parser qual
p_recurrence = p_liftA3 Qrecur (p_pat $p_left p_lArrow) (p_exp $p_left p_comma) p_rangePart

|| generator or qualifying guard, e.g. x < 10
p_qualifier :: parser qual
p_qualifier = p_recurrence $p_alt p_generator $p_alt (p_fmap Qguard p_exp)

p_comprehension :: parser expr
p_comprehension
    = p_inBrackets (p_liftA3 mkCompr p_getLoc (p_exp $p_left p_vBar) (p_someSepBy p_semi p_qualifier))
      where
...
p_rangePart = p_exp $p_left p_sym ".."
...
p_rangeBy
    = p_liftA3 mkRangeBy (p_exp $p_left p_comma) p_rangePart (p_optional p_exp)
      where
...
p_rangeExpr = p_inBrackets (p_range $p_alt p_rangeBy)


|| common parser for simple expressions and definition formals
p_formalSimple :: parser expr
p_formalSimple
    = p_fmap Evar p_var         $p_alt
      p_fmap Evar p_constructor $p_alt
      p_literal

|| simple (atomic) expressions
p_simple :: parser expr
p_simple
    = p_formalSimple    $p_alt
      p_parenExpr p_exp $p_alt
      p_caseExpr        $p_alt
      p_comprehension   $p_alt
      p_listExpr  p_exp $p_alt
      p_postSection     $p_alt
      p_preSection      $p_alt
      p_rangeExpr
...
exprOrNegLit (Eap (Evar op) (Eint n)) = Eint (neg n), if op == stdNegName
exprOrNegLit e                        = e
...
p_expExpr pPre pInf pTerm es ops
    = p_bind2 (p_optional pPre) pTerm fexp
      where
        fexp mop e
            = p_expOp pPre pInf pTerm (e : es) ops'
              where
                ops' = ops,      if isNothing mop
                     = op : ops, otherwise
                op   = fromJust mop
...
p_expOp pPre pInf pTerm es ops
    = (pInf $p_bind fbinop) $p_alt p_expAp pPre pInf pTerm es ops
      where
...
p_expAp pPre pInf pTerm es ops
    = (pTerm $p_bind fexpr) $p_alt p_expFinal es ops
      where
        fexpr e2 = p_expOp pPre pInf pTerm (Eap e1 e2 : es') ops
        e1 : es' = es
...
p_expReduce pPre pInf pTerm es ops
    = p_expReduce pPre pInf pTerm (pe : es1) (opr : ops'), if #es > 0 & #ops > 1 & assocl == AssocPrefix
    = p_expReduce pPre pInf pTerm ces        cops,         if #es > 1 & #ops > 1 & assocl == AssocCompare & assocr == AssocCompare
    = p_expReduce pPre pInf pTerm (re : es2) (opr : ops'), if #es > 1 & #ops > 1 & (precl > precr \/ precl == precr & assocl == AssocLeft)
    = p_expExpr   pPre pInf pTerm es         ops,          otherwise
      where
        er : es1         = es
        el : es2         = es1
        opr : opl : ops' = ops
...
p_casePat
    = p_pat $p_bind checkPat
      where
        ...
        checkPat p = p_pure p,                                 if isCasePat p
                   = p_expected "a restricted simple pattern", otherwise

p_caseAlt :: parser caseAlt
p_caseAlt
    = p_liftA2 CaseAlt p_casePat (p_rArrow $p_right p_rhs)

p_caseExpr :: parser expr
p_caseExpr
    = p_liftA2 (Ecase Cnone) (p_ident "case" $p_right p_exp) (p_ident "of" $p_right p_inLayout (p_some p_caseAlt))

|| parse conditional expression
p_cond :: parser cond
p_cond
    = p_liftA2 mkCond p_exp (p_comma $p_right (p_if $p_alt p_ow))
      where
        p_if         = p_ident "if"        $p_right p_fmap Cif p_exp
        p_ow         = p_ident "otherwise" $p_right p_pure Cdefault
        mkCond e pc  = pc e

|| parse conditional expressions
p_conds :: parser expr
p_conds
    = p_liftA2 mkCond p_cond (p_many (p_outdent $p_right p_equal $p_right p_indent $p_right p_cond))
      where
        mkCond c cs = Econd (c : cs)

|| parse "where ..." definitions
p_whdefs :: parser [defn]
p_whdefs = p_fmap (map fst) (p_ident "where" $p_right p_some p_def)

|| parse right hand side of an expression in a layout block
p_rhs :: parser expr
p_rhs
    = p_inLayout (p_bind2 (p_conds $p_alt p_exp) (p_optional p_whdefs) mkRhs)
      where
...
p_patOrFnForm
    = p_expExpr p_patPrefix p_infix p_formal [] [] $p_bind mkPatFn []
      where
        mkPatFn args (Eap e arg)
            = mkPat [] arg $p_bind next
              where
        ...
        mkPat args e
            = mkPatFn args e $p_bind checkPat
              where
                checkPat (Left p)  = p_pure p
                checkPat (Right x) = patErr

        patErr = p_error 2 "error in pattern definition"

|| parse a pattern only
p_pat :: parser pat
p_pat
    = p_patOrFnForm $p_bind checkPat
      where
        checkPat (Left p)  = p_pure p
        checkPat (Right x) =  p_expected "a pattern"
...
p_tname :: parser name
p_tname
    = p_identifier $p_bind check
      where
        err tn   = "a type name, but got constructor " ++ getName tn
        check tn = p_pure tn,           if isVar tn
                 = p_expected (err tn), if isConstructor tn    || return more information for this common error
                 = p_unexpected,        otherwise
...
p_tinfix
    = (p_symbol $p_guard isTinfix) $p_alt p_unexpected
      where
        isTinfix n
...
p_tform
    = p_liftA2 mkTform p_tname (p_many p_tvar) $p_alt
...
p_parenTexpr
    = mkParen $p_fmap p_inParens (p_listManyOf p_type)
      where
...
p_listTexpr = Tlist $p_fmap p_inBrackets p_type

|| parse a type argument
p_tArg :: parser texpr
p_tArg = p_fmap (converse Tname []) p_tname $p_alt
         p_tvar                             $p_alt
         p_parenTexpr                       $p_alt
         p_listTexpr
...
p_tOp es ops
    = (p_tinfix $p_bind finfix) $p_alt p_tAp es ops
      where
        finfix s = p_tExpr es (s : ops)

|| parse a possible type application (of higher kind) and push it on the expr stack
p_tAp :: [texpr] -> [name] -> parser texpr
p_tAp es ops
    = (p_tArg $p_bind fap e) $p_alt p_tFinal es ops
      where
        e : es'              = es
...
p_cinfix = p_infix $p_guard isConstructor

p_construct :: parser tform
p_construct
    = p_liftA2 mkConstr p_constructor (p_many p_tSArg)       $p_alt
      p_liftA3 mkInfConstr p_type p_cinfix p_type            $p_alt
...
p_spec = p_liftA3 mkVars p_getLoc (p_listSomeOf p_var $p_left p_colons) (p_inLayout p_type) $p_alt
         p_liftA2 mkType p_getLoc (p_listSomeOf p_tform $p_left p_colons $p_left p_ident "type")
         where
...
p_sig :: parser [defn]
p_sig = p_fmap (map fst . concat) (p_some p_spec)
...
p_tdef
    = p_liftA3 mkSyn  p_getLoc (p_tform $p_left p_eqeq)      (p_inLayout p_type)       $p_alt
      p_liftA3 mkData p_getLoc (p_tform $p_left p_sym "::=") (p_inLayout p_constructs) $p_alt 
      p_liftA3 mkAbs  p_getLoc (p_ident "abstype" $p_right p_tform) (p_ident "with" $p_right p_inLayout p_sig)
      where
...
p_def
    = p_liftA3 mkDef p_getLoc p_patOrFnForm (p_equal $p_right p_rhs)
      where
...
p_fileSpec
    = (mkStd $p_fmap p_inAngles p_identifier) $p_alt
      (p_any $p_bind fid)
      where
        mkStd n            = FileSpec stdlibPath (getName n)
        fid (TlitString s) = p_pure (fileSpecFromString s)
        fid t              = p_expected "a file name"

p_libPart :: parser libPart
p_libPart
    = p_getLoc $p_bind floc
      where
        floc loc
            = (p_pure (LibAll loc) $p_left p_sym "+")                        $p_alt
               p_fmap ($LibIdent loc)  p_varOrConstructor                    $p_alt
               p_fmap ($LibRemove loc) (p_minus $p_right p_varOrConstructor) $p_alt
               p_fmap ($LibFile loc)   p_fileSpec

p_libQual :: parser libQual
p_libQual
    = mkQual $p_fmap p_optional (p_ident "qualified")
      where
...
p_libAs
    = mkAs $p_fmap p_optional (p_ident "as" $p_right p_identifier)
      where
...
p_alias
    = p_alias' p_var         $p_alt
      p_alias' p_constructor $p_alt
      p_fmap AliasRemove (p_minus $p_right p_var)
      where
        p_alias' p = p_liftA2 Alias (p $p_left p_sym "/") p

p_binding :: parser libBinding
p_binding = p_liftA2 BindVar  (p_var $p_left p_equal) (p_inLayout p_exp) $p_alt
            p_liftA2 BindType (p_var $p_left p_eqeq)  (p_inLayout p_type)

p_binder :: parser [libBinding]
p_binder = p_inBraces (p_many p_binding)

p_env :: parser libEnv
p_env
    = (p_liftA4 mkEnv p_getLoc p_libQual p_fileSpec p_libAs $p_apply p_optional p_binder) $p_apply p_many p_alias
      where
        mkEnv loc lq fs la mbi as
            = (fs, loc, lq, la, fromMaybe [] mbi, as)

p_libDir :: parser modInserter
p_libDir
    = p_percent $p_right
          (p_fmap modInsInc ((p_ident "include" $p_alt p_ident "import") $p_right p_inLayout p_env) $p_alt
           p_fmap modInsExp (p_ident "export"  $p_right p_inLayout (p_many p_libPart))              $p_alt
           p_fmap modInsFre (p_ident "free"    $p_right p_inBraces p_sig)                           $p_alt
           p_error 2 "unknown library directive")
...
p_decl
    = p_fmap lst         p_libDir $p_alt
      p_fmap (map snd)   p_spec   $p_alt
      p_fmap lst         p_tdef   $p_alt        || try parsing a tdef before trying a def, so that a type synonym isn't incorrectly parsed as an inline fn def
      p_fmap (lst . snd) p_def
      where
        lst x = [x]

p_decls :: parser [modInserter]
p_decls = p_fmap concat (p_many p_decl) $p_left p_end
