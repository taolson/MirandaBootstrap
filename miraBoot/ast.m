...
astAccumLex f a (ds, ls)
    = (ds', ls)
      where
        (ds', ls') = f (astAccumSubs (astAccumLex f)) a (ds, ls)
...
astAccumMap  t = converse (m_fold t)
...
|| astRewriters can be composed like: compose r1 r2 = ($st_bind r1) . r2
...
astRewriteBU r a = astRewriteSubs (astRewriteBU r) a $st_bind r
astRewriteTD r a = r a $st_bind astRewriteSubs (astRewriteTD r)
...
astRewriteLex r a
    = st_get $st_bind doLex
      where
        doLex (ds, ls) = r (astRewriteSubs (astRewriteLex r)) a $st_left (st_modify (converse setSnd ls))
...
astRewriteSubs t (DefAbs tf ds t1 a) = st_liftA3 DefAbs    (t tf)                (st_mapM t ds)  (t t1)      $st_apply (st_pure a)
astRewriteSubs t x                   = st_pure x

|| rewrite a list of asts
astRewriteList :: astRewriter * -> [ast] -> * -> ([ast], *)
astRewriteList = st_mapM

|| rewrite a map of ast elements
astRewriteMap :: astRewriter * -> m_map ** ast -> * -> (m_map ** ast, *)
...
astRewriteInLet :: astRewriter * -> defnMap -> [defn] -> * -> (defnMap, [defn], *)
astRewriteInLet t defs pats s
    = (defs', pats', s')
      where
        dummyExpr                    = Eint 0
        (Elet x1 defs' pats' x2, s') = t (Elet True defs pats dummyExpr) s
